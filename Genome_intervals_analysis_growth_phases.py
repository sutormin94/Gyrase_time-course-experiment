...
#Input data - GCSs, TAB.
path_to_GCSs_files={'Gyrase exponential Micro': "F:\E_coli_Topo-Seqs\GCSs_data\Gyrase_Micro_trusted_GCSs_h_s_0.01.txt",
                    'Gyrase exponential Oxo': "F:\E_coli_Topo-Seqs\GCSs_data\Gyrase_Oxo_trusted_GCSs_h_s_0.01.txt",
                    'Gyrase exponential RifCfx': "F:\E_coli_Topo-Seqs\GCSs_data\Gyrase_RifCfx_trusted_GCSs_h_s_0.01.txt",
                    'Gyrase exponential Cfx': "F:\E_coli_Topo-Seqs\GCSs_data\Gyrase_Cfx_10mkM_trusted_GCSs_h_s_0.01.txt",
...
path_to_TUs_sets={'Exponential phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\Exponential_phase_expression_door_like_no_rRNA.txt",
                  'Early stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\Early_stationary_phase_expression_door_like_no_rRNA.txt",
                  'Stationary phase' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\Stationary_phase_expression_door_like_no_rRNA.txt",
                  'Exponential phase old' : "F:\E_coli_RNA-Seq\E_coli_DY330_RNA-Seq\Expression_data\DOOR_Mu_del_cor_genes_expression_no_rRNA.txt"}

#Length of the genome, bp
Genome_length=4647454

#Path for TU analysis output.
TU_analysis_outpath="F:\Topo_data_new_expression\TU_based_analysis\Gyrase\All_genes_1000bp_no_rRNA\\"
if not os.path.exists(TU_analysis_outpath):
    os.makedirs(TU_analysis_outpath)

#Input data - sets of intervals.
path_to_intervals_sets_1={'BIMEs1': "C:\Sutor\science\DNA-gyrase\scripts\Gyrase_Topo-seq\Additional_genome_features\BIMEs1_coordinates.broadPeak",
...
path_to_intervals_sets_2={'MatP 0': "C:\Sutor\science\DNA-gyrase\scripts\Gyrase_Topo-seq\Additional_genome_features\MatP_sites_37deg_Nolivos_W3110_Mu_SGS.BroadPeak",
...
path_to_intervals_sets_3={'MatP Ter': "C:\Sutor\science\DNA-gyrase\scripts\Gyrase_Topo-seq\Additional_genome_features\MatP_sites_thr_3_37deg_Nolivos_W3110_Mu_SGS_Ter.BroadPeak",
...
def TU_association(GCSs_sets_dict, TU_set, set_type, window_width, path_out, set_name):
    print(f'#####\nGCSs assignment to TUs intervals is going...\n#####')
    fileout=open(path_out + set_name + '_numbers_of_associated_GCSs.txt', 'w')
    fileout_reduced=open(path_out + set_name + '_numbers_of_associated_GCSs_DS_only.txt', 'w')
    if set_type=='16S operons':
        ...
        fileout_reduced.write("Gene_ID\tGene_name\tStart\tEnd\tStrand\tExpression")  
        for GCSs_set_name, GCSs_set_data in GCSs_sets_dict.items():
            fileout.write("\tCondition\tGCSs in USUS\tGCSs in USGB\tGCSs in GBDS\tGCSs in DSDS")
            fileout_reduced.write(f'\t{GCSs_set_name} DSDS')
        fileout.write("\n")
        fileout_reduced.write("\n")
    else:
        print('Unknown type of the set! Expected types: 16S_operons, operons, genes.')
        return
    ...
    for j in TU_set: #j - particular operon info (dictionary)  
        #print(f'Progression: {TU_counter}/{len(TU_set)}\r', flush=True)
        fileout.write(j['TUID'] + '\t' + j['TU name'] + '\t' + str(j['Start']) + '\t' + str(j['End']) + '\t' + j['Strand'] + '\t' + str(j['Transcription level']) + '\t')   
        fileout_reduced.write(j['TUID'] + '\t' + j['TU name'] + '\t' + str(j['Start']) + '\t' + str(j['End']) + '\t' + j['Strand'] + '\t' + str(j['Transcription level']) + '\t')   
        ...
        for a, s in GCSs_sets_dict.items(): #a - Topo-Seq condition, s - corresponding set of GCSs.
            ...
            if set_type=='16S operons':
                ...
                fileout_reduced.write(str(stats[4]) + '\t') #DSDS
                #USUS
                operons_stat_ds[a][0]+=stats[0]
                operons_stat_ds[a][1]+=N3E[0]
                operons_stat_ds[a][2]+=score[0] 
...
def TU_interval_stat_analysis(GCSs_sets_dict, intervals_GCSs_dict, intervals, score_data, window_width, set_type, path_out, set_name, genome_len):
    print(f'#####\nStatistical analysis of GCSs number in TUs intervals is going...\n#####')
...
def GCSs_num_normalization(all_TUs, part_TUs, GCSs_sets_dict, all_TUs_set, part_TUs_set):
    ...
    print(f'#####\nGCSs number normalization is going...\n#####')
...
def write_GCSs_norm(GCSs_set_exp_interval_dict, path_out, set_name):
    print(f'#####\nGCSs assignment data is writing...\n#####')
...
def TU_analysis_wrapper(GCSs_input_dict, score_inpath, TUs_sets_path, path_out, genome_len):
    #Reading input.
    GCSs_sets_dict=trusted_GCSs_parsing(GCSs_input_dict) #Parsing GCSs
    score_data=score_data_parser(score_inpath, 'score') #Parsing score file
    TU_sets_dict=TUs_parser(TUs_sets_path) #Parsing TUs
    window_width=5000
    
    #Genes sets analysis.
    for TU_set_name, TU_set_data in TU_sets_dict.items():
        print(f'#####\nNow analysing the {TU_set_name} TUs set...\n#####')      
        GCSs_genes_assoc_info=TU_association(GCSs_sets_dict, TU_set_data, 'genes', window_width, path_out, TU_set_name)
        TU_interval_stat_analysis(GCSs_sets_dict, GCSs_genes_assoc_info, TU_set_data, score_data, window_width, 'genes', path_out, TU_set_name, genome_len)
        GCSs_set_exp_interval_dict_g=GCSs_num_normalization(GCSs_genes_assoc_info, GCSs_genes_assoc_info, GCSs_sets_dict, TU_set_data, TU_set_data)
        write_GCSs_norm(GCSs_set_exp_interval_dict_g, path_out, TU_set_name)
    return
